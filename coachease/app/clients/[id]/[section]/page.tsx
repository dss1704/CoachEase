...
export default async function ClientSection({ params }: { params: Promise<{ id: string; section: string }> }) {
  ...
  const { data, error } = await supabase.from("clients").select("id, name, goal, coach_notes, starting_weight, current_weight, target_weight").eq("id", id).eq("coach_id", user.id).maybeSingle();
